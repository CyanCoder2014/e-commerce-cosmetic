        ...
        <div class="timeline">
            ...
            <div class="timeline-detail">
                <div class="timeline-head">
                    <h3>&nbsp; <a target="_blank" href="<?= Url('home/profile/show/137-'.$post->user->id.'-42-'.$post->user->username); ?>" id="popoverone"  data-content="
                                           @if($post->user->profile !== null)
                    ...
                    @if(Auth::check())
                        ...
                        @can('update', $post)
                            <div style="float: left" class="social-share">
                                <a class="" title="ویرایش"><i class="ti-pencil-alt "></i></a>
                            </div>
                        @endcan
                    @endif
                </div>
                        ...
                        <li style="margin-left: 20px !important;">
                            <div><span><img style="height: 30px; border: solid 1px #5e5e5e ; border-radius: 3px"
                                ...
                                <span href="<?= Url('home/profile/show/137-'.$post->answered($post->parent_id)->user->id.'-42-'.$post->answered($post->parent_id)->user->username); ?>" title="{{$post->answered($post->parent_id)->user->name.' '.$post->answered($post->parent_id)->user->family}}"><a
                                           target="_blank" href="<?= Url('home/profile/show/137-'.$post->answered($post->parent_id)->user->id.'-42-'.$post->answered($post->parent_id)->user->username); ?>"
                                ...
                                @if($post->answered($post->parent_id)->image_b !== 'null' && $post->answered($post->parent_id)->image_b !== 'file' && $post->answered($post->parent_id)->image_b !== '')
                                    <div id="file-1234-{{$post->id}}" style=" max-height: 400px;overflow: hidden">
                                        <a target="_blank" href="<?= Url('/post-files/'.$post->answered($post->parent_id)->image_b); ?>" type="button" name="_token"
                ...
                <div class="timeline-content">
                    ...
                    <p   id="content-{{$post->id}}">
                        {!! \Illuminate\Support\Str::words($post->intro, $words = 190, $end = '...') !!}
                    ...
                    @if($post->image_b !== 'null' && $post->image_b !== 'file' && $post->image_b !== '')
                        <div id="file-1234-{{$post->id}}" style=" max-height: 400px;overflow: hidden">
                            <a target="_blank" href="<?= Url('/post-files/'.$post->image_b); ?>" type="button" name="_token"
                    ...
                    <div id="post-comment-first<?php echo $post->id?>">
                        @foreach($post->comments as $comment)
                            ...
                            <span class="margin-12" id="popoverthree" rel="popover" data-placement="top"
                                ...
                                @endcan
                            @endif
                            <a target="_blank" href="<?= Url('home/profile/show/137-'.$comment->user->id.'-42-'.$comment->user->username); ?>" title="">&nbsp;{{$comment->user->username}} </a>
                            <p>{{$comment->comment}}   </p>
                                    ...
                                    @foreach($comment->answers as $reply)
                                     ...
                                     <span class="tooltip2"><span
                                         ...
                                         @endif
                                                ...
                                                <span  title=""><a target="_blank" href="<?= Url('home/profile/show/137-'.$reply->user->id.'-42-'.$reply->user->username); ?>"
