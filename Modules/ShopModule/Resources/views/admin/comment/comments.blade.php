                                                    ...
                                                    @foreach($comments as $comment)
                                                        @if($comment->approved == 0)
                                                            <tr class="odd gradeX">
                                                                ...
                                                                <th style="text-align: center; font-weight: 100">
                                                                    @if($comment->product)
                                                                    <?php $post=$comment->product;?>
                                                                    <a href="{{ route('shop.show',['id'=>'353-'.$post->id.'-'.$post->name['fa']])}}">{{$post->name['fa']}}</a>
                                                                    @endif
                                                                </th>
                                                                <th style="text-align: center; font-weight: 100">
                                                                    @can('apply pcomment')
                                                                    @if($comment->approved == 0)
                                                                        <a   href="{{ route('product.comment.accept',['id' =>$comment->id]) }}" class="btn btn-success"><i class="fa fa-check"></i> تایید</a>
                                                                    @elseif($comment->approved == 1)
                                                                        <a   href="{{ route('product.comment.deny',['id' =>$comment->id]) }}" class="btn btn-warning"><i class="fa fa-remove"></i>رد</a>
                                                                    @endif
                                                                    @endcan
                                                                ...
                                                                <th style="text-align: center; font-weight: 100">
                                                                    @can('delete pcomment')
                                                                    <a onclick="return confirm('آیا از حذف این محصول مطمئن هستید؟');"  href="{{ route('product.comment.delete',['id' =>$comment->id]) }}" class="btn btn-danger"><i class="fa fa-remove"></i></a>
                                                                    @endcan
                                                                </th>
            ...
            <div class="modal-dialog modal-lg" role="document">
                <div class="modal-content">
                    ...
                    <form name="_token" method="POST" enctype="multipart/form-data"
                          action="{{ route('product.comment.reply') }}">
                        {{ csrf_field() }}
